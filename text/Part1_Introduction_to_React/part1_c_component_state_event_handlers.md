...
Reactの公式チュートリアルでは, この規約を提案しています.


## Changes in state cause rerendering
アプリケーションがどのように機能するかという主要な原則について見ていきましょう.

アプリケーションが起動すると, `App`のコードが実行されます.
このコードは, `useState`フックを使用してアプリケーションの状態を作成し,
`counter`変数の初期値を設定します.
このコンポーネントには, カウンターの値0を表示する`Display`コンポーネントと3つの`Button`コンポーネントが含まれています.
すべてのボタンにはイベントハンドラーがあり, カウンターの状態を変更するために使用されます.

ボタンの1つがクリックされると, イベントハンドラが実行されます.
イベントハンドラは, `setCounter`関数を使用して`App`コンポーネントの状態を変更します.
状態を変更する関数を呼び出すと, コンポーネントが再レンダリングされます.

したがって, ユーザがplusボタンをクリックすると, ボタンのイベントハンドラが`counter`の値を1に変更し,
`App`コンポーネントが再レンダリングされます.
これにより, サブコンポーネントの`Display`および`Button`も再レンダリングされます.
